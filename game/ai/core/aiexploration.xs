...
minInterval 30
{
   ...
   switch (islandExploreMode)
   {
      ...
      case cIslandExploreModeStart:
      {
         ...
         if (scoutUnitID < 0)
         {
            if (aiPlanGetIDByIndex(cPlanTransport, -1, true, 0) < 0 && gActiveIslandInvasion == false)
            {
               scoutType = findBestScoutType();
               scoutUnitID = getUnit(scoutType, cMyID, cUnitStateAlive);
               if (scoutUnitID >= 0)
               {
                  planID = createTransportPlan(kbBaseGetMilitaryGatherPoint(cMyID, kbBaseGetMainID(cMyID)), islandExplorePosition, 80); // Was 100...
                  if (planID >= 0)
                  {
