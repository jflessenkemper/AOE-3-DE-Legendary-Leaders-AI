...
bool handleExcessResources()
{
   ...
   // if (age == cAge2 && gAgeUpResearchPlan < 0)
   // {
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceGold, 1000.0);
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceWood, 0.0);
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceFood, 1200.0);
   ...
   else if ((age < cAge4) && (gAgeUpResearchPlan < 0)) // Enhanced - && (gAgeUpResearchPlan < 0)
   {
...
void updateResourceDistribution(bool force = false)
{
   ...
   // if (kbGetAge() == cAge2) {
   //    aiSetResourceGathererPercentage(cResourceGold, 0.45);
   //    aiSetResourceGathererPercentage(cResourceFood, 0.50);
   //    aiSetResourceGathererPercentage(cResourceWood, 0.05);
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceGold, 1000.0);
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceWood, 0.0);
   //    xsArraySetFloat(gExtraResourceNeeds, cResourceFood, 1200.0);
   //    xsArraySetFloat(gRawResourcePercentages, cResourceGold, 0.400);
   //    xsArraySetFloat(gRawResourcePercentages, cResourceWood, 0.100);
   //    xsArraySetFloat(gRawResourcePercentages, cResourceFood, 0.500);
   // } else {
   ...
   // Enhanced
   // if (kbGetAge() == cAge2) {
   //    goldPercentage = 0.450;
   //    woodPercentage = 0.050;
   //    foodPercentage = 0.500;
   // }
   ...
   // Let's age faster!
   // if (kbGetAge() == cAge2) {
   //    gGatherPlanPriorityHunt = 95;
   //    gGatherPlanPriorityBerry = 8;
   //    gGatherPlanPriorityMill = 0;
   //    gGatherPlanPriorityWood = 0;
   //    gGatherPlanPriorityMine = 85;
   //    gGatherPlanPriorityEstate = 0;
   //    gGatherPlanPriorityFish = 20;
   //    gGatherPlanPriorityWhale = 19;
   // }
   ...
   //    }
   //    if (kbGetAge() == cAge2) {
   //       aiSetResourcePercentage(cResourceGold, false, 0.45);
   //       aiSetResourcePercentage(cResourceWood, false, 0.05);
   //       aiSetResourcePercentage(cResourceFood, false, 0.50);
   //    }
...
{
   // In Age 2, after two minutes, stop gathering wood until we age up.
   // if (kbGetAge() == cAge2) { //  && gAgeUpResearchPlan < 0
   //    gGatherPlanNumWoodPlans = 0; // && (xsGetTime() > gAgeUpTime + 120 * 1000 || xsGetTime() > 120 * 1000)
   //    return; //&& aiPlanGetActive(gAgeUpResearchPlan) == false
   // }
   // We don't want more gatherers for wood; we need to focus on getting to Age 3.
   // if (kbGetAge() == cAge2 && 
   //    aiGetNumberGatherers(cUnitTypeAbstractVillager, cResourceWood, cAIResourceSubTypeEasy) > 4) {
...
minInterval 15
{
   ...
   vector mainBaseVec = cInvalidVector;
   int time = xsGetTime();
   // Don't slaughter livestock early on.
   if ((kbGetAge() < cAge3))
   {
      return;
   }
   ...
   mainBaseVec = kbBaseGetLocation(cMyID, kbBaseGetMainID(cMyID));
   numLivestock = getUnitCountByLocation(cUnitTypeHerdable, cPlayerRelationAny, cUnitStateAny, mainBaseVec, 960.0);
   ...
   {
      slaughterPlanID = aiPlanCreate("Livestock slaughter", cPlanGather);
      ...
      // 2-4 gatherers if there is livestock.
      aiPlanAddUnitType(slaughterPlanID, gEconUnit, gatherersWanted, gatherersWanted, 2 * gatherersWanted);
      aiPlanSetDesiredPriority(slaughterPlanID, 100);
      aiPlanSetActive(slaughterPlanID);
      debugEconomy("Activated livestock slaughter plan " + slaughterPlanID);
   }
   else
   {
      // 2-4 gatherers if there is livestock
   ...
   // Adjust priority based on livestock count
   if (numLivestock > 7)
   {
      aiPlanSetDesiredPriority(slaughterPlanID, 100);
   }
   else if (numLivestock <= 2)
   {
      aiPlanSetDesiredPriority(slaughterPlanID, 60);
   }
   
   if (aiPlanGetState(slaughterPlanID) == -1)
   {
      debugEconomy("Livestock gather plan " + slaughterPlanID + " is invalid");
